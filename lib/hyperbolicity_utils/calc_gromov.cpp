...
// Calculates the gromov four_point condition.  Excuse the large
// sections of commented code, this function's final form is still in
// flux.
vector< vector<double> > calc_gromov_quads(Graph& G, v_size_t diam, int quads_to_keep, int dist_keep, string quad_output, bool single)
{
  ...
  ////////////////////////////////////////////////////////////
  //Pointers to shared data arrays that have copy of the delta
  //information for each thread, ie, if there are 8 threads, there
  //will be an 8 (pointers to) arrays each accessed by only one
  //thread, At the end of the parallel section of code, the 8 arrays
  //will be combined into a single array
  ///////////////////////////////////////////////////////////
  ...
  vector<bool> quad_found(true,diam);
  //OMP parallel region, this current implementation generates tasks
  //at two of the four levels of the nested for loop, these tasks are
  //then passed to threads which execute them and update their private
  //copy of the results array After the tasks have all been generated
  //and executed the results are collated
#pragma omp parallel shared(delta,maxDelta,shared_max_delta,diam,shared_delta,quad_found) //private(i,j,k,l,u,v,x,y,ui,vi,xi,yi,s1,s2,s3,currDelta,maxPair,delta_loc,maxDelta_loc)
  {
    ...
    // The code below is ugly.  I'm sorry.
#pragma omp single
    {
      for(size_t i=0;i<size;++i)
	{
#pragma omp task shared(shared_delta,shared_max_delta,d,vert_vec,index,quad_found)
	  {
	    for(size_t j=i+1;j<size;++j)
	      {
#pragma omp task shared(shared_delta,shared_max_delta,d,vert_vec,index,quad_found)
			...
			for(size_t l=k+1;l<size;++l)
			  {
			    ...
			    if(quad_found[delta_ind] && maxPair>=dist_keep)
			      {
				for(int thr_id=0;thr_id < quads_kept[delta_ind].size();++thr_id)
				  {
				    delta_count += quads_kept[delta_ind][thr_id];
				  }
				if(delta_count>=quads_to_keep)
				  quad_found[delta_ind]=false;
			      }
			    
			    
			    // Enter if a quad found with appropriate parameters
			    if(quad_found[delta_ind] && maxPair>=dist_keep)
			      {
				...
				// Find all shortest paths if !single,
				...
				if(!single)
				  ...
				  {
				    BFS_source_target_single(u,v,G,u_v,pred);
				    BFS_source_target_single(u,x,G,u_x,pred);
				    BFS_source_target_single(u,y,G,u_y,pred);
				    BFS_source_target_single(v,x,G,v_x,pred);
				    BFS_source_target_single(v,y,G,v_y,pred);
				    BFS_source_target_single(x,y,G,x_y,pred);
				  }
				...
				// Produce subgraph from quadruplet
				Graph Gnew = subset(G,master);

				string quad_output_curr = quad_output;

				// Get names correct.  ie translate
				// quad indices into filenames
				char delt_char[40];
				...
				if(!single)
				  ...
				  quad_orig_indices.append("_single_orig_indices.txt");
				...
				if(!single)
				  ...
				  {
				    quad_orig_core.append("_single_orig_k_core.color");
				    quad_output_curr.append("_single.dimacs");
				  }
				...
				// Write out quadruped, a file
				// containing the original indices in
				// the original graph, and then a
				// color file from the original graph.
				// The write_graphviz function can now
				// used these three files to produce a
				// colored visualization from a color
				// vector generated on the original
				// network
			  ...
			  for(vwi=0;vwi<vw.size();++vwi)
			    {
			      ...
			      //Is there a way to confirm the load
			      //balancing issue on this?  For example,
			      //I could set the predecessors to a
			      //fixed set of nodes and see what
			      //happens each time?  I could give
			      //everyone a set of the predecessor
			      //matrix and see how it scales in that
			      //case.
